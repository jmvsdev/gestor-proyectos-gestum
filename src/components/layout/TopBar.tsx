    ...
    <div className="flex flex-col flex-shrink-0">
      ...
      <div className="flex items-center gap-[10px] px-[18px] pt-[10px] pb-[6px]">
        <div className="flex items-center gap-2 flex-1 min-w-0 overflow-hidden">
          ...
          <span className="text-[#c3c7d0] text-[14px]">/</span>
          <LayoutDashboard size={15} strokeWidth={2} className="flex-shrink-0" style={{ color: accentColor }} />
          <span className="text-[14px] font-semibold text-[#272b36] truncate">{activePhase}</span>
          <Star size={15} strokeWidth={1.8} className="text-[#c3c7d0] flex-shrink-0 cursor-pointer hover:text-amber-400 transition-colors" />
        </div>

        <button
          type="button"
          onClick={onShare}
          className="flex items-center gap-[6px] border-0 rounded-lg px-[13px] py-[7px] text-[13px] font-semibold text-white cursor-pointer ml-1 shadow-sm hover:brightness-110 transition-all flex-shrink-0"
          style={{ background: accentColor }}
        >
      ...
      {/* Row 2: view tabs */}
      <div className="flex items-center gap-0.5 px-[18px] border-b border-gray-200">
        {TABS.map(({ label, Icon }) => {
